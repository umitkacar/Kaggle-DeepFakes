...
while(1):
    ...
    if xMeta.label[j] == 'FAKE':
        ...
        filename = '/home/umit/xDataset/deepFake-dat/Train_Fake/' +xMeta.axes[0][j].split('.')[0]
                        ...
                        for multi_face_original in range(1): #range(len(face_original)):
                            ...
                            if percent_pixel > IMG_PERCENT_PIXEL_THRESHOLD:
                                ...
                                try:
                                    ...
                                    crop = cv2.cvtColor(crop_fake_X, cv2.COLOR_BGR2RGB)
                                    image_rgb = np.transpose(crop, (2, 0, 1))
                                    frame_vec = image_rgb.reshape(IMG_SIZE*IMG_SIZE*3,)
                                    f.write(frame_vec)
                                    
                                    # Write dmap with .dat format
                                    dmap_file = cv2.resize(temp, (DMAP_SIZE, DMAP_SIZE))*2
                                    dmap_file = np.uint8(dmap_file)
                                    dmap_file = np.expand_dims(dmap_file,axis=2)
                                    dmap_file = np.transpose(dmap_file, (2, 0, 1))
                                    dmap_vec = dmap_file.reshape(DMAP_SIZE*DMAP_SIZE*1,)
                                    f.write(dmap_vec)
                                    
                                    # Write label with .dat format
                                    label = np.ones((1))
                                    label = np.uint8(label)
                                    label_vec = label.reshape(1,)
                                    f.write(label_vec)
                ...
                if (ret_real == True):
                    ...
                    if face_real: # empty control
                        ...
                        for multi_face_real in range(1): #range(len(face_real)):
                            ...
                            # Write image with .dat format
                            crop = cv2.cvtColor(crop_real_X, cv2.COLOR_BGR2RGB)
                            image_rgb = np.transpose(crop, (2, 0, 1))
                            frame_vec = image_rgb.reshape(IMG_SIZE*IMG_SIZE*3,)
                            f.write(frame_vec)
                            
                            # Write dmap with .dat format
                            dmap_file= np.zeros((DMAP_SIZE,DMAP_SIZE,1))
                            dmap_file = np.uint8(dmap_file)
                            dmap_file = np.transpose(dmap_file, (2, 0, 1))
                            dmap_vec = dmap_file.reshape(DMAP_SIZE*DMAP_SIZE*1,)
                            f.write(dmap_vec)
